	...
	public function index()
	{
		return View::make('adm.sync.summary.index', [
			'summary' => SyncDb::selectRaw(
					...
					(SELECT COUNT(*) FROM sync_db WHERE sync_db.item_id IS NOT NULL AND sync_db.dev_id = dev.id) AS count_insert_prod,
					(SELECT COUNT(*) FROM sync_db WHERE sync_db.dev_id = dev.id) AS count_items_dev'
				)
