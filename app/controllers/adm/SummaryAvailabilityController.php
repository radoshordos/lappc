    ...
    public function index()
    {
        if (Request::isMethod('post'))
        {
            foreach (Input::get('visible') as $key => $val) {
                DB::update('UPDATE items_availability SET visible = ? WHERE id = ?',array($val,$key));
            }
        }

        return View::make('adm.summary.availability.index', array(
            'ia' => DB::table('items_availability')
                ->select(array('items_availability.*',
