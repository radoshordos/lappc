...
@section('content')
    {{ Form::open(['route' => ['adm.stats.buyorderdevs.index'], 'method' => 'GET', 'class' => 'form-horizontal', 'role' => 'form']) }}
        <table class="table">
            ...
            <tbody>
                <tr>
                    <td>{{ Form::input('month', 'month_start', $choice_month_start, ['class' => 'form-control', 'required' => 'required', 'min' => $min_insert_month, 'max' => date('Y-m-d')]) }}</td>
                    <td>{{ Form::input('month', 'month_end', $choice_month_end , ['class' => 'form-control','required' => 'required', 'min' => $min_insert_month, 'max' => date('Y-m-d') ]) }}</td>
